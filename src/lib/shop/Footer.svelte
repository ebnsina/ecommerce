...
<footer class="mt-16 border-t border-border bg-surface-alt">
	...
	<!-- Six columns, not five: the brand block takes two and there are four
	     lists to place — Shop plus the three saved in Menus. At five, the last
	     one wrapped onto a row of its own with the width of the page beside it. -->
	<div class="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:grid-cols-2 lg:grid-cols-6">
	...
	<!-- One last line: who owns the shop at the left, what it takes at the
	     right. The payment badges were a band of their own above this, which
	     left two thin rules and a strip of nothing between them. -->
	<div class="border-t border-border">
		<div
			class="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-x-6 gap-y-3 px-4 py-5 text-xs text-ink-muted"
		>
			...
			{#if badges.length}
				<ul class="flex flex-wrap items-center gap-2">
					{#each badges as m (m.name)}
						<li
							class="flex h-8 items-center gap-2 rounded-lg border border-border bg-surface px-2.5"
							title={m.name}
						>
							{#if m.logo}
								<!-- The label stays as the accessible name; the logo is the picture of it. -->
								<img src={m.logo} alt={m.name} class="h-4 w-auto max-w-14 object-contain" />
							{:else}
