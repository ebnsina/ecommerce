<script lang="ts">
	...
	const links = [
		{ href: '/demo/account', label: 'Overview', Icon: User },
		{ href: '/demo/account/orders', label: 'My orders', Icon: Package },
		{ href: '/demo/account/wishlist', label: 'Wishlist', Icon: Heart },
		{ href: '/demo/account/addresses', label: 'Addresses', Icon: MapPin }
	];

	const active = (href: string) =>
		href === '/demo/account'
			? page.url.pathname === '/demo/account'
...
<div class="mx-auto grid max-w-7xl gap-6 px-4 py-8 lg:grid-cols-[16rem_1fr]">
	<aside class="h-fit rounded-3xl border border-border bg-surface p-3">
		...
		<form method="POST" action="/demo/logout" class="mt-2 border-t border-border pt-2">
			<button
