...
<div class="mt-6 grid gap-4 sm:grid-cols-3">
	{#each [{ label: 'Orders', value: data.recent.length, href: '/demo/account/orders' }, { label: 'Wishlist', value: data.wishlistCount, href: '/demo/account/wishlist' }, { label: 'Addresses', value: data.addressCount, href: '/demo/account/addresses' }] as s (s.label)}
		<a
			href={s.href}
...
<section class="mt-4 rounded-3xl border border-border bg-surface">
	<div class="flex items-center justify-between border-b border-border px-5 py-4">
		...
		<a href="/demo/account/orders" class="text-sm text-primary">All orders</a>
	</div>
	{#if data.recent.length}
		{#each data.recent as o (o.id)}
			<a
				href="/demo/account/orders/{o.id}"
