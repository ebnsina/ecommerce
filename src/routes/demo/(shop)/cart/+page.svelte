...
<div class="mx-auto max-w-7xl px-4 py-8">
	...
	{#if data.lines.length === 0}
		<div class="mt-6 flex flex-col items-center gap-4 rounded-3xl border border-border px-6 py-20">
			...
			<Button href="/demo/search">Start shopping</Button>
		</div>
	{:else}
		<div class="mt-6 grid gap-6 lg:grid-cols-[1fr_20rem]">
				...
				{#each groups.loose as line (line.id)}
					...
					>
						...
						<div class="flex min-w-0 flex-1 flex-col gap-1">
							<a href="/demo/p/{line.slug}" class="text-sm font-medium text-ink hover:text-primary">
								{line.title}
							</a>
							{#if line.optionLabel}
							...
							{#each group.lines as line (line.id)}
								<li class="flex items-center gap-3">
									...
									<span class="min-w-0 flex-1">
										<a href="/demo/p/{line.slug}" class="block truncate text-sm text-ink"
											>{line.title}</a
										>
			...
			<aside class="h-fit rounded-3xl border border-border bg-surface p-5 lg:sticky lg:top-28">
				...
				<Button href="/demo/checkout" block class="mt-5" disabled={data.problems.length > 0}>
					Checkout
					<ArrowRight size={16} />
				</Button>
